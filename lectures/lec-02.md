...
```mermaid
graph TB
    subgraph Pool1[Організація]
        subgraph Lane1[Служба безпеки]
            A[Виявлення витоку] --> B[Первинна оцінка]
            B --> C{Паралельний шлюз}
            C --> D[Технічне розслідування]
            F --> G{Злиття}
            E --> G
            D --> G
            G --> H[Підготовка звіту]
        end
        subgraph Lane2[Юридичний відділ]
            C --> E[Юридична оцінка]
        end
        subgraph Lane3[PR-департамент]
            C --> F[Підготовка заяви]
        end
    end
```
...
```mermaid
graph TB
    Start1([Тривога від системи]) --> EventGW{Подієвий шлюз}
    Start2([Регулярна перевірка]) --> EventGW
    Start3([Звернення користувача]) --> EventGW

    EventGW --> Class[Первинна класифікація]
    Class --> CritGW{Критичність?}

    CritGW -->|Висока| Emergency[Екстрене реагування]
    CritGW -->|Середня| Standard[Стандартне розслідування]
    CritGW -->|Низька| Queue[Реєстрація в чергу]

    Emergency --> Merge{Інклюзивне злиття}
    Standard --> Merge
    Queue --> Merge

    Merge --> Doc[Документування]
    Doc --> End([Завершення])
```
